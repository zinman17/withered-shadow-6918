...
async function catalog(ctx) {
    ...
    if (ctx.method === 'POST' && ctx.fields.buy_item) {
        if (ctx.user === null) {
            ...
            return ctx.redirect('login?next=colors');
        }
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        const action = ownedNow
            ...
            : '<form method="post" action="colors" class="inlineform">' + L.csrfField(ctx) + '<input type="hidden" name="item_id" value="' + Number(item.id) + '"><button class="Button YesButton" type="submit" name="buy_item" value="1">Buy</button></form>';
    ...
    const money = bal !== null
        ...
        : '<p class="CatMoney center"><a href="login?next=colors">Log in</a> to buy items. New members get 100 Tix and 25 Brux.</p>\n';
    const body = '<div class="bottombar panel">\n<h2 class="center">Body Colors</h2>\n' +
    ...
    return page(ctx, 'Body Colors - WallOfBricks', body);
}
